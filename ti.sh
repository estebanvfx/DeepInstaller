...
dependence(){
  ...
  if [ "$Full" == '1' ]; then
    echo -ne "\n\033[31mError! \033[0mPlease use '\033[33mapt-get\033[0m' to install the missing dependencies.\n\n\n"
    exit 1;
  fi
}
...
getDisk(){
  disks=$(lsblk -nd -o NAME | grep -v "loop\|sr\|fd" | head -n1)
  if [ -n "$disks" ]; then
    echo "/dev/$disks"
  else
    echo ""
...
getGrub(){
  ...
  [ -n "$folder" ] || return
  fileName=$(ls -1 "$folder" 2>/dev/null | grep -E '^grub\.cfg$')
  if [ -z "$fileName" ]; then
    return
  fi
...
if [ "$setNet" == "0" ]; then
  ...
  iAddr=$(ip addr show dev $interface | grep "inet " | awk '{print $2}' | head -n1)
  ipAddr=$(echo ${iAddr} | cut -d'/' -f1)
  ipMask=$(netmask $(echo ${iAddr} | cut -d'/' -f2))
  ipGate=$(ip route show default | grep "^default" | awk '{print $3}')
fi
if [ -z "$interface" ]; then
    dependence "ip"
    [ -n "$interface" ] || interface=$(getInterface)
...
if [ "$isRecovery" = "yes" ];then
  ...
  while [ -z $validDD ]; do
    ...
    read DDURL
    validDD=$(validUrl $DDURL)
    [ -z $validDD ] && echo 'Please input a valid URL. Only http://, ftp://, and https:// are supported!'
  done
fi
clear && echo -e "\n\033[36m# Install\033[0m\n"
yesno="n"
echo "Installer will reboot your computer then re-install using this information"
if [ "$isDebug" = "yes" ];then
  REBOOT=""
fi
if [ -n "$DDURL" ]; then
...
if [ ! -f $GRUBDIR/$GRUBFILE ];then
  echo "Grub config not found at $GRUBDIR/$GRUBFILE. TinyInstaller only runs on Debian or Ubuntu!"
  exit 2
fi
echo ""
...
echo "Preparing GRUB..."
INSERTGRUB=$(grep -n '### BEGIN /etc/grub.d/30_os-prober ###' $GRUBDIR/$GRUBFILE | cut -d: -f1 | head -n1)
if [ "$INSERTGRUB" != "" ];then
  sed -i ''${INSERTGRUB}'r /tmp/grub.new' $GRUBDIR/$GRUBFILE;
else
  echo "Failed to find insert location for GRUB configuration."
  exit 1
fi

if [ "$isDebug" = "yes" ];then
  echo "Debug mode enabled, not rebooting."
  exit 0
else
